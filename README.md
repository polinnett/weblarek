...
`modal:close` – клик на фон, клик на крестик. Назначение: закрыть модальное окно и сбросить состояние preview.
...
4. Презентер вызывает рендер компонентов только в двух ситуациях: после изменения данных в модели (изменение корзины и тд) или при открытии модального окна (предпросмотр товара и тд).
