...
3. Презентер обрабатывает все пользовательские действия:
   ...
   открытие превью товара: `catalog:select`.
   действие в превью товара: `preview:action`.
   открытие корзины: `basket:open`.
   удаление из корзины: `basket:remove`.
   переход к оформлению: `basket:checkout`.
   изменение полей формы: `form:change`.
   отправка формы заказа: `order:submit`.
   отправка формы контактов: `contacts:submit`.
